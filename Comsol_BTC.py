...
frac_BTC =  pd.read_csv('fracture_outlet_C_6mL.txt', skiprows=5, delim_whitespace=True, names=['Time', 'Concentration (mol)'])
chan_BTC =  pd.read_csv('channel_outlet_C_6mL.txt', skiprows=5, delim_whitespace=True, names=['Time', 'Concentration (mol)'])
x = BTC.iloc[:,0] #get the time array in seconds
fig1, axs = plt.subplots(3,1 , figsize=(12,9))
fig1.suptitle('COMSOL BTC; 0.09mL/min', fontsize=20)
xlabel = 'Time [s]'
...
fig2.suptitle('COMSOL BTC; 0.09mL/min Semilog', fontsize=20)
...
x = frac.iloc[:,0]
fig, ax = plt.subplots(figsize=(12,9), dpi=800)
plt.plot(x, chan.iloc[:,1], label = '')
plt.plot(x, frac.iloc[:,1])
ax.semilogy()
plt.title('Tracer Distribution Throughout Fracture, Semilog', fontsize=24)
plt.tick_params(axis='x', which='major', labelsize=16) # x-axis tick size
plt.tick_params(axis='y', which='major', labelsize=16) # y-axis tick size
